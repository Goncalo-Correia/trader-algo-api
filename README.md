...
| `Database__MaxPoolSize` | No | `5` | Local Npgsql connection-pool cap for the API process. Keep this well below the Supabase session-pool `pool_size` (15 on the small pooler) so background workers queue locally instead of exhausting Supabase client slots shared with the ML sidecar. |
| `Database__MinPoolSize` | No | unset | Optional minimum number of local Npgsql connections to keep open. Leave unset unless cold-start latency matters; it must be less than or equal to `Database__MaxPoolSize`. |
| `Database__ConnectionIdleLifetimeSeconds` | No | `30` | How quickly idle pooled Npgsql connections become eligible for pruning. Keep short with Supabase session pooling so idle API traffic releases server slots promptly. |
| `Database__ConnectionPruningIntervalSeconds` | No | `10` | How often Npgsql checks for idle pooled connections to prune. |
